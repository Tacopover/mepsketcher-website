...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { trial_user_email, license_id }: AssignTrialUserRequest =
      await req.json();
    ...
    if (licenseData.used_licenses >= licenseData.total_licenses) {
      return new Response(
    ...
    const { data: trialUser, error: trialUserError } = await supabaseAdmin
      .from("users")
      .select(
        "id, organization_id, organizations(id, name, is_personal_trial_org)"
      )
    ...
    // Increment used_licenses on the license
    const { error: incrementSeatsError } = await supabaseAdmin
      .from("organization_licenses")
      .update({ used_licenses: licenseData.used_licenses + 1 })
      .eq("id", license_id);

    if (incrementSeatsError) {
      console.error("Error incrementing used_licenses:", incrementSeatsError);
