...
serve(async (req) => {
  try {
    ...
    // Filter orgs that have no users
    const orgsToDelete = orphanedOrgs?.filter(
      (org: any) => !org.users || org.users.length === 0
    ) || [];

    console.log(`Found ${orgsToDelete.length} orphaned personal trial orgs to clean up`);
    ...
    for (const org of orgsToDelete) {
      ...
      if (deleteError) {
        ...
        console.log(`Successfully deleted orphaned org: ${org.name} (${org.id})`);
      }
    }

    console.log(`Cleanup complete. Deleted ${deletedCount} of ${orgsToDelete.length} orphaned orgs`);
